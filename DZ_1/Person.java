...
public class Person {
    ...
    public static status setStatus(String status) {
        ...
        motherList.add("мама");
        motherList.add("мать");
        motherList.add("мамуля");
        motherList.add("mother");
        motherList.add("mom");
        motherList.add("mommy");
        fatherList.add("папа");
        fatherList.add("отец");
        fatherList.add("батя");
        fatherList.add("father");
        fatherList.add("dad");
        fatherList.add("daddy");
        daughterList.add("дочь");
        daughterList.add("daughter");
        daughterList.add("доча");
        daughterList.add("доченька");
        daughterList.add("дочурка");
        sonList.add("сын");
        sonList.add("сынуля");
        sonList.add("сынок");
        sonList.add("сыночек");
        ...
        for (String s : motherList) {
            ...
            if (Objects.equals(s, status)) {
                thisStatus = DZ_1.status.mother;
            }
        }
        for (String s : fatherList) {
            s = s.toLowerCase();
            if (Objects.equals(s, status)) {
                thisStatus = DZ_1.status.father;
            }
        }
        for (String s : daughterList) {
            s = s.toLowerCase();
            if (Objects.equals(s, status)) {
                thisStatus = DZ_1.status.daughter;
            }
        }
        for (String s : sonList) {
            s = s.toLowerCase();
            if (Objects.equals(s, status)) {
                thisStatus = DZ_1.status.son;
            }
        }
        return thisStatus;
    }
